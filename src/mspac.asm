...
0005  c30b23    jp      j_230b		; jump to startup test 
...
000a  10fc      djnz    j_0008           ; loop until B == #00
000c  c9        ret     		; return

	; arrive here from #23A7

000d  c30e07    jp      j_070e		; sets up difficulty
...
002e  1812      jr      j_0042           ; continue this sub below
...
0035: C3 51 00	jp	j_0051		; continue this sub below
...
j_0038:
0038  c39b1f	jp      j_1f9b		; patched jump from pacman.
003b  ----50 				; junk from pac-man
003c  320750    ld      (#5007),a	; junk from pac-man
003f  c33800    jp      j_0038		; junk from pac-man
...
0049  2002      jr      nz,j_004d	; If non zero, skip next step
...
0052: A7	and	a		; == #00 ?
0053: 28 06	jr	z,j_005b 	; yes, skip ahead, we will insert the new task here

0055: 1C	inc	e		; else inc E by 3
0056: 1C	inc	e
0057: 1C	inc	e		; DE now at next task
0058: 10 F7	djnz	j_0051		; Next B, loops up to #10 times
005A: C9	ret			; return
...
0119  2038      jr      nz,j_0153        ; no , skip ahead
...
0157  ca7601    jp      z,j_0176		; no, skip ahead
...
018c  cddc01    call    j_01dc		; update all timers
018f  cd2102    call    j_0221		; check timed tasks and execute them if it is time to do so
0192  cdc803    call    j_03c8		; runs subprograms based on game mode, power-on stuff, attract mode, push start screen, and core loops for game playing
0195  3a004e    ld      a,(#4e00)	; load A with game mode
0198  a7        and     a		; is the game still in power-on mode ?
0199  2812      jr      z,j_01ad         ; yes, skip over next calls

019B  cd9d03    call    j_039d		; check for double size pacman in intermission (pac-man only)
019E  cd9014    call    j_1490		; when player 1 or 2 is played without cockatil mode, update all sprites
01a1  cd1f14    call    j_141f		; when player 2 is played on cockatil mode, update all sprites
01a4  cd6702    call    j_0267		; debounce rack input / add credits
01a7  cdad02    call    j_02ad		; debounce coin input / add credits
01aa  cdfd02    call    j_02fd		; blink coin lights
...
01b1  2006      jr      nz,j_01b9        ; no, skip next 2 steps		; set to jr #01b9 to enable sound in demo
...
j_01b9:
01b9    call    j_2d0c                   ; process effects
01bc    call    j_2cc1                   ; process waves
...
01ca  2808      jr      z,j_01d4         ; yes, skip ahead
...
01d1  ca0000    jp      z,j_0000		; yes, reset
...
01ef  2013      jr      nz,j_0204        ; if not equal, break out of loop
...
01fa  2008      jr      nz,j_0204        ; if not equal, break out of loop
...
022C: 28 2F	jr	z,j_025d		; Yes, jump ahead and loop for next task
...
0233: 30 28	jr	nc,j_025d	; if no, jump ahead and loop for next task
...
0239: 20 22	jr	nz,j_025d	; no, jump ahead and loop for next task
...
0283  ccdf02    call    z,j_02df		; If yes, call #02df  ; add credit
...
0293  c29a02    jp      nz,j_029a	; no, skip ahead
...
02f1  d2f602    jp      nc,j_02f6	; if no carry, skip ahead
...
0304  201f      jr      nz,j_0325        ; skip ahead if result is not zero
...
0317  3002      jr      nc,j_031b        ; if no carry then skip next 2 steps
0319  af        xor     a		; A := #00
031a  4f        ld      c,a		; c := #00

j_031b:
031b  2801      jr      z,j_031e         ; If zero then skip next step
...
0332  ca4403    jp      z,j_0344		; Yes, Jump ahead

0335  3a034e    ld      a,(#4e03)	; else load A with main routine 2, subroutine #
0338  fe02      cp      #02		; <= 2 ?
033a  d24403    jp      nc,j_0344	; yes, skip ahead

033d  cd6903    call    j_0369		; else draw "1UP"
0340  cd7603    call    j_0376		; draw "2UP"
0343  c9        ret     		; return
...
034b  c25903    jp      nz,j_0359	; 

034e  cb67      bit     4,a		; test bit 4 of the counter.  is it on?
0350  cc6903    call    z,j_0369		; no, draw  "1UP"
0353  c48303    call    nz,j_0383	; yes, clear "1UP"
0356  c36103    jp      j_0361		; skip ahead

j_0359:
0359  cb67      bit     4,a		; test bit 4 of the counter.  is it on?
035b  cc7603    call    z,j_0376		; no, draw  "2UP"
035e  c49003    call    nz,j_0390	; yes, clear "2UP"

j_0361:
0361  3a704e    ld      a,(#4e70)	; load A with player# (0=player1, 1=player2)
0364  a7        and     a		; is this player 1 ?
0365  cc9003    call    z,j_0390		; yes, clear "2UP"
0368  c9        ret			; return     
...
03fe  cda12b    call    j_2ba1		; write # of credits on screen
0401  3a6e4e    ld      a,(#4e6e)	; load A with # of credits
0404  a7        and     a		; == #00 ?
0405  280c      jr      z,j_0413         ; yes, skip ahead
...
JP ATTRACT
j_0413:
0413  c35c3e    jp      j_3e5c		; jump to mspac patch when there are no credits - controls the demo mode
...
046d  cd8505    call    j_0585		; draw text to screen, increase subroutine #
...
0476  cdbf05    call    j_05bf		; draw stationary red ghost on screen
0479  0e0c      ld      c,#0c		; load C with text code for "CHARACTER / NICKNAME"
047b  cd8505    call    j_0585		; insert task to write text to screen
047e  c9        ret     		; return

047f  0e14      ld      c,#14		; load C with text code for "-SHADOW"
0481  cd9305    call    j_0593		; draw text to screen
0484  c9        ret     		; return

0485  0e0d      ld      c,#0d		; load C with text code for ""BLINKY""
0487  cd9305    call    j_0593		; draw text to screen
048a  c9        ret     		; return
...
0490  cdbf05    call    j_05bf		; draw stationary pink ghost on screen
0493  0e0c      ld      c,#0c		; load C with text code for "CHARACTER / NICKNAME"
0495  cd8505    call    j_0585		; insert task to write text to screen
0498  c9        ret     		; return

0499  0e16      ld      c,#16		; load C with text code for "-SPEEDY"
049b  cd9305    call    j_0593		; draw text to screen
049e  c9        ret     		; return

049f  0e0f      ld      c,#0f		; load C with text code for ""PINKY""
04a1  cd9305    call    j_0593		; draw text to screen
04a4  c9        ret     		; return
...
04aa  cdbf05    call    j_05bf		; draw stationary inky on screen
04ad  0e0c      ld      c,#0c		; load C with text code for "CHARACTER / NICKNAME"
04af  cd8505    call    j_0585		; insert task to write text to screen
04b2  c9        ret     		; return

04b3  0e33      ld      c,#33		; load C with text code for "-BASHFUL"
04b5  cd9305    call    j_0593		; draw text to screen
04b8  c9        ret     		; return

04b9  0e2f      ld      c,#2f		; load C with text code for ""INKY""
04bb  cd9305    call    j_0593		; draw text to screen
04be  c9        ret     		; return
...
04c4  cdbf05    call    j_05bf		; draw stationary orange ghost on screen
04c7  0e0c      ld      c,#0c		; load C with text code for "CHARACTER / NICKNAME"
04c9  cd8505    call    j_0585		; insert task to write text to screen
04cc  c9        ret     		; return

04cd  0e35      ld      c,#35		; load C with text code for "-POKEY"
04cf  cd9305    call    j_0593		; draw text on screen
04d2  c9        ret     		; return

04d3  0e31      ld      c,#31		; load C with text code for ""CLYDE""
04d5  c38005    jp      j_0580		; draw text and increase game mode

04d8  ef        rst     #28		; insert task to write text ". 10 Pts"
04d9  1c 11
04da  0e12      ld      c,#12
04dd  c38505    jp      j_0585		; insert task to write text "o 50 Pts"

04e0  0e13      ld      c,#13		; load C with text code for "(C) MIDWAY MFG CO"
04e2  cd8505    call    j_0585		; insert task to write text to screen
04e5  cd7908    call    j_0879		; setup game start variables
04e8  35        dec     (hl)
...
0525  2005      jr      nz,j_052c        ; no, skip ahead and do a core loop
0527  3601      ld      (hl),#01	; yes, change ghost substate to going for pac-man
0529  c38e05    jp      j_058e		; jump ahead, increase game state and return

	; a core game loop used in pac-man demo mode only, not used in ms. pac

j_052c:
052c  cd1710    call    j_1017		; another core game loop which does many things
052f  cd1710    call    j_1017		; another core game loop which does many things
0532  cd230e    call    j_0e23		; change animation of ghosts every 8th frame
0535  cd0d0c    call    j_0c0d		; handle power pill flashes
0538  cdd60b    call    j_0bd6		; set ghost colors
053b  cda505    call    j_05a5		; check for direction reversal after eating power pill
053e  cdfe1e    call    j_1efe		; check for red ghost direction reversal
0541  cd251f    call    j_1f25		; check for pink ghost direction reversal
0544  cd4c1f    call    j_1f4c		; check for blue ghost (inky) direction reversal
0547  cd731f    call    j_1f73		; check for orange ghost direction reversal
054a  c9        ret     
...
0553  c32405    jp      j_0524		; check for reaching position to release next ghost
...
055e  c32405    jp      j_0524		; check for reaching position to release next ghost
...
0569  c32405    jp      j_0524		; check for reaching position to release next ghost
...
0576  ca8e05    jp      z,j_058e		; yes, skip ahead and increase subroutine number

0579  c32c05    jp      j_052c		; no, loop back again to core loop

; arrive here in demo mode from #3ECD

j_057c:
057c  cdbe06    call    j_06be		; jump to new subroutine based on game state
...
0587  cd4200    call    j_0042		; insert task to display text, parameter = variable text
...
059a  cd4200    call    j_0042		; set task to display ghost name
...
05a1  cd8e05    call    j_058e		; increase main routine 1, subroutine # (#4E02)
05a4  c9        ret     		; return
...
05F3: CD A1 2B	call	j_2ba1		; write # of credits on screen
...
061b  cda12b    call    j_2ba1		; write # of credits on screen
...
0625  2002      jr      nz,j_0629        ; if >= 2 credits, skip next step
0627  0608      ld      b,#08		; load B with message #8:  "1 PLAYER ONLY"
j_0629:
0629  cd5e2c    call    j_2c5e		; print message
...
0634  280c      jr      z,j_0642         ; don't check p2 with 1 credit
0636  cb77      bit     6,a		; check for player 2 start button
0638  2008      jr      nz,j_0642        ; if not, pressed, skip ahead to check for player 1 start
063a  3e01      ld      a,#01		; else set 2 players
063c  32704e    ld      (#4e70),a	; store into # of players (0=1 player, 1=2 players)
063f  c34906    jp      j_0649		; jump ahead
j_0642:
...
064d  2815      jr      z,j_0664         ; Yes, skip ahead
...
0656  2803      jr      z,j_065b         ; If 1 player game, skip ahead and only subtract 1 credit
...
0661  cda12b    call    j_2ba1		; write # of credits on screen
...
06ac  cd6a2b    call    j_2b6a		; draw remaining lives at bottom of screen 
06af  af        xor     a		; A := #00
...
070f  a7        and     a		; == #00 ?
0710  2004      jr      nz,j_0716        ; no, skip ahead
...
0737  cd1408    call    j_0814		; copy data into #4d46 through #4d94
...
074d  cd3a08    call    j_083a		; copy difficulty info to #4DB8 to #4DBA
...
0792  cdea2b    call    j_2bea		; draw fruit at bottom of screen
...
0880  cdc924    call    j_24c9		; set up pills and power pills in RAM
...
089d  2006      jr      nz,j_08a5        ; no, skip ahead
...
08ca  c39408    jp      j_0894		; loop back to increment level complete register and return

; demo or game is playing

08cd  3a0050    ld      a,(#5000)	; load A with IN0
08d0  cb67      bit     4,a		; is rack test on?
08d2  c2de08    jp      nz,j_08de	; no, skip ahead
...
JP MOREDOTS
NOP
08e1  c3a194    jp      j_94a1		; jump to ms pac man new check for end of level routine
08e4  00        nop 			; junk
...
j_08eb:
08eb  cd1710    call    j_1017		; another core game loop that does many things
08ee  cd1710    call    j_1017		; another core game loop that does many things
08f1  cddd13    call    j_13dd		; check for release of ghosts from ghost house
08f4  cd420c    call    j_0c42		; adjust movement of ghosts if moving out of ghost house
08f7  cd230e    call    j_0e23		; change animation of ghosts every 8th frame
08fa  cd360e    call    j_0e36		; periodically reverse ghost direction based on difficulty (only when energizer not active)
08fd  cdc30a    call    j_0ac3		; handle ghost flashing and colors when power pills are eaten
0900  cdd60b    call    j_0bd6		; color dead ghosts the correct colors
0903  cd0d0c    call    j_0c0d		; handle power pill (dot) flashes
0906  cd6c0e    call    j_0e6c		; change the background sound based on # of pills eaten
0909: CD AD 0E	call	j_0ead		; check for fruit to come out.  (new ms. pac sub actually at #86EE.)
...
0912  cd8724    call    j_2487		; save pellet info to memory
...
091c  a7        and     a		; == #00 ?
091d  201f      jr      nz,j_093e        ; no, skip ahead
...
0933  cd4200    call    j_0042		; insert task to draw to screen either "PLAYER ONE" or "PLAYER TWO"
...
0952  cda12b    call    j_2ba1		; else draw # credits or free play on bottom of screen
...
j_0961:
0961  cda60a    call    j_0aa6		; transposes data from #4e0a through #4e37 into #4e38 through #4e66
...
09ae  2806      jr      z,j_09b6         ; Yes, skip ahead
...
09be  2804      jr      z,j_09c4         ; yes, skip next step
...
09cf  c39408    jp      j_0894		; increase main routine # and return from sub
...
09ec  cd4200    call    j_0042		; set task #01 with parameter #02, or task #01 with parameter #00
...
09f6  cd7e26    call    j_267e		; clears all ghosts from screen
...
0a00  18e8      jr      j_09ea           ; flash screen

0a02  18e4      jr      j_09e8           ; flash screen

0a04  18f8      jr      j_09fe           ; flash screen

0a06  18e0      jr      j_09e8           ; flash screen

0a08  18f4      jr      j_09fe           ; flash screen

0a0a  18dc      jr      j_09e8           ; flash screen

0a0c  18f0      jr      j_09fe           ; flash screen
...
0A33: 18 06         jr   j_0a3b		; skip next 2 steps
...
0a3e  fe14      cp      #14		; > #14 ?
0a40  3802      jr      c,j_0a44         ; no, skip next step
...
0a89  cdc924    call    j_24c9		; set addresses #4E16 through #4E39 with #FF and #14 (pill map???)
...
0aa0  c38809    jp      j_0988		; 

; ; called from #06C1 when (#4E04 == #22)

0aa3  c3d209    jp      j_09d2		; 
...
0ad6  c2d20b    jp      nz,j_0bd2	; no, decrement (IY) and return
...
0ae1  281b      jr      z,j_0afe         ; no, skip ahead
...
0ae9  3013      jr      nc,j_0afe        ; no, skip ahead
...
0AF5: 20 04	jr	nz,j_0afb	; no, skip ahead
...
0b02  281d      jr      z,j_0b21         ; no, skip ahead and set red ghost to red
...
0b37  281d      jr      z,j_0b56         ; no, skip ahead and set pink ghost to pink
...
0b6c  281d      jr      z,j_0b8b         ; no, skip ahead
...
0ba1  281d      jr      z,j_0bc0         ; no, skip ahead
...
0bdd  c2e20b    jp      nz,j_0be2	; no, skip next step
...
0bea  caf00b    jp      z,j_0bf0		; yes, skip next step. only set color if not alive
...
0bf4  cafa0b    jp      z,j_0bfa		; yes, skip next step
...
0bfe  ca040c    jp      z,j_0c04		; yes, skip next step
...
0c1c  2015      jr      nz,j_0c33        ; no, skip ahead and flash the pellets in the demo screen where pac is chased by 4 ghosts and then eats a power pill and eats them all

; BUGFIX05 - Map discoloration fix - Don Hodges
0c1c  2000	jr 	nz,j_0c1e	; no, do nothing
...
ORG 0C21H
JP FLASHEN
0c21  c32495    jp      j_9524		; jump to new ms pac routine to flash power pellets
...
0c24  2002      jr      nz,j_0c28        ; no, skip ahead
...
0c39  2002      jr      nz,j_0c3d        ; no, skip next step
...
0c53  c2900c    jp      nz,j_0c90	; yes, skip ahead and check next ghost
...
0c5e  cd0020    call    j_2000		; load HL with IY + IX = new position by moving up
...
0c71  c2900c    jp      nz,j_0c90	; no, skip ahead and check next ghost
...
0c95  cafb0c    jp      z,j_0cfb		; yes, skip ahead and check next ghost

0c98  fe00      cp      #00		; is pink ghost waiting to leave the ghost house?
0c9a  c2c10c    jp      nz,j_0cc1	; no, skip ahead
...
0ca2  cc2e1f    call    z,j_1f2e		; yes, reverse direction of pink ghost

0ca5  fe80      cp      #80		; is pink ghost at bottom of the ghost house?
0ca7  cc2e1f    call    z,j_1f2e		; yes, reverse direction of pink ghost
...
0cb8  cd0020    call    j_2000		; load HL with IX + IY = new pink ghost position
0cbb  22024d    ld      (#4d02),hl	; store into pink ghost position
0cbe  c3fb0c    jp      j_0cfb		; skip ahead and check next ghost
...
0cc9  cd0020    call    j_2000		; load HL with IY + IX = new pink ghost position
...
0cdc  c2fb0c    jp      nz,j_0cfb	; no, skip ahead and check next ghost
...
0d00  ca930d    jp      z,j_0d93		; yes, skip ahead and check next ghost

0d03  fe00      cp      #00		; is inky waiting to leave the ghost house ?
0d05  c22c0d    jp      nz,j_0d2c	; no, skip ahead
...
0d0d  cc551f    call    z,j_1f55		; yes, reverse direction of inky
0d10  fe80      cp      #80		; is inky at the bottom of the ghost house ?
0d12  cc551f    call    z,j_1f55		; yes, reverse direction of inky
...
0d23  cd0020    call    j_2000		; load HL with IX + IY = new inky position
0d26  22044d    ld      (#4d04),hl	; store into inky position
0d29  c3930d    jp      j_0d93		; skip ahead and check next ghost
...
0d31  c2590d    jp      nz,j_0d59	; no, skip ahead
...
0d3c  cd0020    call    j_2000		; load HL with IX + IY = new inky position
...
0d61  cd0020    call    j_2000		; load HL with IX + IY = new inky position
...
0d74  c2930d    jp      nz,j_0d93	; no, skip ahead and check next ghost
...
0d9b  c2c00d    jp      nz,j_0dc0	; no, skip ahead
...
0da3  cc7c1f    call    z,j_1f7c		; yes, reverse orange ghost direction

0da6  fe80      cp      #80		; is orange ghost at bottom of ghost house ?
0da8  cc7c1f    call    z,j_1f7c		; yes, reverse orange ghost direction
...
0db9  cd0020    call    j_2000		; load HL with IX + IY = new orange ghost position
...
0dc5  c2ea0d    jp      nz,j_0dea	; no, skip ahead
...
0dd0  cd0020    call    j_2000		; load HL with IX + IY = new orange ghost position
...
0df2  cd0020    call    j_2000		; load HL with IX + IY = new orange ghost position
...
0e70  2805      jr      z,j_0e77         ; no, skip ahead
...
0E7F: FE E4	cp	#E4		; > #E4 ?
0E81: 38 06	jr	c,j_0e89		; no, skip ahead
...
0e8b  3806      jr      c,j_0e93         ; no, skip ahead
...
0e95  3806      jr      c,j_0e9d         ; no, skip ahead
...
0e9f  3806      jr      c,j_0ea7         ; no, skip ahead
...
JP DOFRUIT
j_0ead:
0ead  c3ee86    jp      j_86ee		; jump to Ms. Pac patch for fruit release OTTO DOFRUIT
...
0eba  fe46      cp      #46		; == #46 ?
0ebc  280e      jr      z,j_0ecc         ; yes, skip ahead to check for launch of first fruit
0ebe  feaa      cp      #aa		; == #AA ?
...
0eca  1809      jr      j_0ed5           ; skip ahead to release fruit
...
0ee3  3802      jr      c,j_0ee7         ; yes, skip ahead
...
100B: C3 78 36	jp	j_3678		; ms pac patch to erase the fruit score
...
j_1017:
1017  cd9112    call    j_1291		; do things if pacman is dead
...
101f  cd6610    call    j_1066		; check for ghosts being eaten and set ghost states accordingly
1022  cd9410    call    j_1094		; check for red ghost state and do things if not alive
1025  cd9e10    call    j_109e		; check for pink ghost state and do things if not alive
1028  cda810    call    j_10a8		; check for blue ghost (inky) state and do things if not alive
102b  cdb410    call    j_10b4		; check for orange ghost state and do things if not alive
102e  3aa44d    ld      a,(#4da4)	; load A with # of ghost killed but no collision for yet [0..4]
1031  a7        and     a		; == #00 ?
1032  ca3910    jp      z,j_1039		; yes, skip ahead

1035  cd3512    call    j_1235		; no, call this sub
1038  c9        ret     		; and return

j_1039:
1039  cd1d17    call    j_171d		; check for collision with regular ghosts
103c  cd8917    call    j_1789		; check for collision with blue ghosts
...
1044  cd0618    call    j_1806		; handle all pac-man movement
1047  cd361b    call    j_1b36		; control movement for red ghost
104a  cd4b1c    call    j_1c4b		; control movement for pink ghost
104d  cd221d    call    j_1d22		; control movement for blue ghost (inky)
1050  cdf91d    call    j_1df9		; control movement for orange ghost
1053  3a044e    ld      a,(#4e04)	; load A with level state subroutine #
1056  fe03      cp      #03		; is a game being played ?
1058  c0        ret     nz		; no, return

1059  cd7613    call    j_1376		; control blue ghost timer and reset ghosts when it is over or when pac eats all blue ghosts
105c  cd6920    call    j_2069		; check for pink ghost to leave the ghost house
105f  cd8c20    call    j_208c		; check for blue ghost (inky) to leave the ghost house
1062  cdaf20    call    j_20af		; check for orange ghost to leave the ghost house
1065  c9        ret     		; return
...
106c  2008      jr      nz,j_1076        ; no, skip ahead and check next ghost
...
1077  2008      jr      nz,j_1081        ; no, skip ahead and check next ghost
...
1082  2008      jr      nz,j_108c        ; no, skip ahead
...
10C0: CD D8 1B	call	j_1bd8		; handle red ghost movement
...
10da  cd0020    call    j_2000		; HL := (IX) + (IY)
...
1118  cdaf1c    call    j_1caf		; handle pink ghost movement
...
1132  cd0020    call    j_2000		; HL := (IX) + (IY)
...
1159  c30111    jp      j_1101		; jump to check for clearing eyes sound

; arrive here from #10AB when blue ghost (inky) is dead (eyes)

115c  cd861d    call    j_1d86		; handle inky movement
...
1176  cd0020    call    j_2000		; HL := (IX) + (IY)
...
1197  cd0020    call    j_2000		; HL := (IX) + (IY)
...
11c6  c30111    jp      j_1101		; jump to check for clearing eyes sound

; arrive here from #10B7 when orange ghost is dead (eyes)

11c9  cd5d1e    call    j_1e5d		; handle orange ghost movement
...
11e3  cd0020    call    j_2000		; HL := (IX) + (IY)
...
1204  cd0020    call    j_2000		; HL := (IX) + (IY)
...
1232  c30111    jp      j_1101		; jump to check for clearing eyes sound
...
124e  2027      jr      nz,j_1277        ; no, skip ahead
...
125f  2804      jr      z,j_1265         ; no, skip next 2 steps
...
12ce  cd7e26    call    j_267e		; clears #4d00 through #4d07
...
12df  2804      jr      z,j_12e5         ; no, skip ahead
...
1350  c3d612    jp      j_12d6		; animate dead mspac
...
136e  cd7526    call    j_2675		; clears all ghosts
...
138b  ca9813    jp      z,j_1398		; if all ghosts are not blue, then skip ahead and reset power pill effect
...
13a1  c2a713    jp      nz,j_13a7	; yes, skip next step
...
13ab  c2b113    jp      nz,j_13b1	; yes, skip next step
...
13b5  c2bb13    jp      nz,j_13bb	; yes, skip next step
...
13bf  c2c513    jp      nz,j_13c5	; yes, skip next step
...
13e4  caee13    jp      z,j_13ee		; yes, skip ahead
...
1408  cc8620    call    z,j_2086		; yes, then call this sub which will release the pink ghost
...
1412  cca920    call    z,j_20a9		; yes, then call this sub which will release Inky
...
141b  ccd120    call    z,j_20d1		; yes, then call this sub which will release orange ghost
141e  c9        ret     		; return
...
148d  c3fe14    jp      j_14fe		; jump ahead
...
1502  c24b15    jp      nz,j_154b	; yes, jump ahead
...
1509  c2b415    jp      nz,j_15b4	; yes, jump ahead
...
151e  282b      jr      z,j_154b         ; no, skip ahead
...
1579  2006      jr      nz,j_1581        ; no, skip next 3 steps

157b  3aa84d    ld      a,(#4da8)	; load A with pink ghost blue flag
157e  a7        and     a		; is pink ghost blue (edible) ?
157f  2009      jr      nz,j_158a        ; yes, skip ahead and check next ghost
...
158e  2006      jr      nz,j_1596        ; no, skip next 3 steps

1590  3aa94d    ld      a,(#4da9)	; load A with inky blue flag
1593  a7        and     a		; is inky edible ?
1594  2009      jr      nz,j_159f        ; yes, skip ahead and check next ghost
...
j_15b4:
15b4  cde615    call    j_15e6		; check for and handle big pac-man sprites in 1st cutscene (pac-man only)
15b7  cd2d16    call    j_162d		; check for and handle sprites in 2nd cutscene (pac-man only)
15ba  cd5216    call    j_1652		; check for and handle sprites in 3rd cutscene (pac-man only)
15bd  78        ld      a,b		; A := B
...
15ef  e60f      and     #0f
15f1  fe0c      cp      #0c
15f3  3804      jr      c,j_15f9         ; (4)

15f5  1618      ld      d,#18
15f7  1812      jr      j_160b           ; (18)

j_15f9:
15f9  fe08      cp      #08
15fb  3804      jr      c,j_1601         ; (4)

15fd  1614      ld      d,#14
15ff  180a      jr      j_160b           ; (10)

j_1601:
1601  fe04      cp      #04
1603  3804      jr      c,j_1609         ; (4)

1605  1610      ld      d,#10
1607  1802      jr      j_160b           ; (2)
...
; MOVING EAST
168c  c39c86    jp      j_869c		; jump to ms. pacman patch to animate ms pac
168f  c9        ret     

1690  07				; junk from pac-man    
1691  fe06      cp      #06
1693  3805      jr      c,j_169a         ;
1695  dd360a30  ld      (ix+#0a),#30
1699  c9        ret     

j_169a:
169a  fe04      cp      #04
169c  3805      jr      c,j_16a3         ;
169e  dd360a2e  ld      (ix+#0a),#2e
16a2  c9        ret     

j_16a3:
16a3  fe02      cp      #02
16a5  3805      jr      c,j_16ac         ;
...
; MOVING SOUTH
16b1  c3b186    jp      j_86b1		; jump to ms. pacman patch to animate ms pac
16b4  c9        ret     

16b5  07				; junk from pac-man    
16b6  fe06      cp      #06
16b8  3805      jr      c,j_16bf         ;
16ba  dd360a2f  ld      (ix+#0a),#2f
16be  c9        ret     

j_16bf:
16bf  fe04      cp      #04
16c1  3805      jr      c,j_16c8         ;
16c3  dd360a2d  ld      (ix+#0a),#2d
16c7  c9        ret     

j_16c8:
16c8  fe02      cp      #02
16ca  3805      jr      c,j_16d1         ;
...
16d9  c3c586    jp      j_86c5		; jump to ms. pacman patch to animate ms pac
16dc  c9        ret     

16dd  3808      jr      c,j_16e7         ;
j_16df:
16df  1e2e      ld      e,#2e
j_16e1:
16e1  cbfb      set     7,e
...
j_16e7:
16e7  fe04      cp      #04
16e9  3804      jr      c,j_16ef         ;
16eb  1e2c      ld      e,#2c
16ed  18f2      jr      j_16e1           ;
j_16ef:
16ef  fe02      cp      #02

16f1  30ec      jr      nc,j_16df        ;
16f3  1e30      ld      e,#30
16f5  18ea      jr      j_16e1           ;
...
16fa  c3d986    jp      j_86d9		; jump to ms. pacman patch to animate ms pac

16fd  c9        ret     

16fe  3805      jr      c,j_1705         ;
1700  dd360a30  ld      (ix+#0a),#30
1704  c9        ret     

j_1705:
1705  fe04      cp      #04
1707  3808      jr      c,j_1711         ; (8)
1709  1e2f      ld      e,#2f
j_170b:
170b  cbf3      set     6,e
...
j_1711:
1711  fe02      cp      #02
1713  3804      jr      c,j_1719         ; (4)

1715  1e2d      ld      e,#2d
1717  18f2      jr      j_170b           ; (-14)

j_1719:
1719  1e2f      ld      e,#2f
171b  18ee      jr      j_170b           ; (-18)
...
1727  2009      jr      nz,j_1732        ; no, skip ahead for next ghost
...
172f  ca6317    jp      z,j_1763		; yes, jump ahead and continue checks

j_1732:
1732  05        dec     b		; B := #03
1733  3aae4d    ld      a,(#4dae)	; load A with blue ghost (inky) state
1736  a7        and     a		; is inky alive ?
1737  2009      jr      nz,j_1742        ; no, skip ahead for next ghost
...
173f  ca6317    jp      z,j_1763		; yes, jump ahead and continue checks

j_1742:
1742  05        dec     b		; B := #02
1743  3aad4d    ld      a,(#4dad)	; load A with pink ghost state
1746  a7        and     a		; is pink ghost alive ?
1747  2009      jr      nz,j_1752        ; no, skip ahead
...
174f  ca6317    jp      z,j_1763		; yes, jump ahead and continue checks

j_1752:
1752  05        dec     b		; B := #01
1753  3aac4d    ld      a,(#4dac)	; load A with red ghost state
1756  a7        and     a		; is red ghost alive ?
1757  2009      jr      nz,j_1762        ; no, skip ahead
...
175f  ca6317    jp      z,j_1763		; yes, jump ahead and continue checks
...
1780  cd5a2a    call    j_2a5a		; update score.  B has code for items scored. draws score on screen, checks for high score and extra lives
...
180c  ca1118    jp      z,j_1811		; no, skip ahead
...
1815  ca2f18    jp      z,j_182f		; no, skip ahead
...
182c  c34318    jp      j_1843		; skip ahead to move pacman
...
1859  3809      jr      c,j_1864         ; yes, skip ahead to handle tunnel movement
...
185f  3003      jr      nc,j_1864        ; yes, skip ahead to handle tunnel movement

1861  c3ab18    jp      j_18ab		; no tunnel movement.  jump ahead to handle normal movement
...
186e  ca191a    jp      z,j_1a19		; yes, skip ahead [ zero this instruction to NOP's to enable playing in demo mode (part 1/2) ] 

1871  3a044e    ld      a,(#4e04)	; else load A with subroutine #
1874  fe10      cp      #10		; <=#10 ?
1876  d2191a    jp      nc,j_1a19	; no, skip ahead
...
187b  2806      jr      z,j_1883         ; No, skip ahead and check IN0
...
1880  c38618    jp      j_1886		; skip ahead
...
1888  c29918    jp      nz,j_1899	; no, skip ahead
...
1896  c35019    jp      j_1950		; jump back to program

j_1899:
1899  cb57      bit     2,a		; is joystick pushed to right?
189b  c25019    jp      nz,j_1950	; no, skip ahead
...
18a8  c35019    jp      j_1950		; jump back to program
...
18b0  ca191a    jp      z,j_1a19		; yes, skip ahead [ zero this instruction into NOP's to enable playable demo mode, (part 2/2) ]

18b3  3a044e    ld      a,(#4e04)	; else load A with subroutine #
18b6  fe10      cp      #10		; <= #10 ?
18b8  d2191a    jp      nc,j_1a19	; no, skip ahead

18bb  79        ld      a,c		; A := C
18bc  a7        and     a		; is this player 2 and cocktail mode ?
18bd  2806      jr      z,j_18c5         ; yes, skip next 2 steps
...
18c2  c3c818    jp      j_18c8		; skip next step
...
18ca  cac91a    jp      z,j_1ac9		; yes, jump to process

18cd  cb57      bit     2,a		; joystick pressed right?
18cf  cad91a    jp      z,j_1ad9		; yes, jump to process

18d2  cb47      bit     0,a		; joystick pressed up?
18d4  cae81a    jp      z,j_1ae8		; yes, jump to process

18d7  cb5f      bit     3,a		; joystick pressed down?
18d9  caf81a    jp      z,j_1af8		; yes, jump to process
...
18ec  cd0f20    call    j_200f		; load A with screen value of position computed in (IX) + (IY)
18ef  e6c0      and     #c0		; mask bits
18f1  d6c0      sub     #c0		; subtract.  is the maze blocking pacman from moving this way?
18f3  204b      jr      nz,j_1940        ; no, skip ahead

18f5  05        dec     b		; yes, was the joystick moved ?
18f6  c21619    jp      nz,j_1916	; yes, skip ahead
...
18fd  da0b19    jp      c,j_190b		; yes, skip next 5 steps
...
1908  c34019    jp      j_1940		; else skip ahead
...
1927  da3519    jp      c,j_1935		; yes, skip next 5 steps
...
1932  c35019    jp      j_1950		; no, skip ahead
...
193d  c35019    jp      j_1950		; no, jump ahead
...
1947  ca5019    jp      z,j_1950		; no, skip ahead
...
1967  ca8519    jp      z,j_1985		; yes, skip ahead

196a  da7119    jp      c,j_1971		; was the last comparison less than #04 ?, if yes, skip next 2 steps

; cornering up to the left or up to the right

196d  2d        dec     l		; lower the X position
196e  c38519    jp      j_1985		; skip ahead

; cornering right from down , cornering left from down

j_1971:
1971  2c        inc     l		; else increase the X position
1972  c38519    jp      j_1985		; skip ahead
...
197a  ca8519    jp      z,j_1985		; yes, skip ahead

197d  da8419    jp      c,j_1984		; was the last comparison less than #04 ?, if yes, skip next 2 steps

; cornering up from the left side, or down from the left side

1980  25        dec     h		; else lower the Y position 
1981  c38519    jp      j_1985		; skip ahead
...
1988  cd1820    call    j_2018		; convert sprite position into a tile position
...
ORG 19ADH
JP EATFRUIT
19AD: C3 18 88	jp	j_8818		; MS Pac-man patch. jump to check for fruit being eaten

19B0: 20 1B	jr	nz,j_19cd	; junk from pac-man
...
19B5: CD 42 00	call	j_0042		; set task #19 with parameter variable A.  updates score.  B has code for items scored, draw score on screen, check for high score and extra lives
19B8: CD 00 10	call	j_1000		; clear fruit.  clears #4DD4 and returns
19BB: 18 07	jr	j_19c4		; skip ahead.  a fruit has been eaten
...
19BE: CD 42 00	call	j_0042		; pac-man only
19C1: CD 04 10	call	j_1004		; pac-man only
...
19D5: CD 65 00	call	j_0065		; load HL with pacman's grid position
19D8: 7E	ld	a,(hl)		; load A with item on grid
19D9: FE 10	cp	#10		; is a dot being eaten ?
19DB: 28 03	jr	z,j_19e0		; yes, skip ahead
...
19F3: CD 42 00	call	j_0042		; set task #19 with variable parameter
...
19F9: CA FD 19	jp	z,j_19fd		; yes, skip next step

19FC: 87	add  a,a		; else it was an energizer. double A to 6

j_19fd:
19FD: 32 9D 4D	ld	(#4D9D),a	; store A to delay update pacman movement
1A00: CD 08 1B	call	j_1b08		; update timers for ghosts to leave ghost house
1A03: CD 6A 1A	call	j_1a6a		; check for energizer eaten
...
1A0D: 38 05	jr	c,j_1a14		; if carry then use other sound pattern
...
1a4a  cd0020    call    j_2000		; load HL with new position of pacman
...
1a64  cd0020    call    j_2000		; load HL with new position of pacman
1a67  c38519    jp      j_1985		; jump to movement check
...
1ad6  c3e418    jp      j_18e4		; return to program
...
1ae5  c3e418    jp      j_18e4		; return to program
...
1af5  c3e418    jp      j_18e4		; return to program
...
1b05  c3e418    jp      j_18e4		; return to program
...
1b0c  ca141b    jp      z,j_1b14		; no, skip ahead
...
1b1d  ca251b    jp      z,j_1b25		; yes, skip ahead
...
1b29  ca311b    jp      z,j_1b31		; yes, skip ahead
...
1b40  cdd720    call    j_20d7		; checks for and sets the difficulty flags based on number of pellets eaten
...
1b49  cd5a20    call    j_205a		; check to see if red ghost has entered a tunnel slowdown area
...
1b50  ca6a1b    jp      z,j_1b6a		; no, skip ahead
...
1bc1  c3d81b    jp      j_1bd8		; skip ahead
...
1bdd  caed1b    jp      z,j_1bed		; yes, skip ahead
...
1be7  caf71b    jp      z,j_1bf7		; yes, skip ahead
1bea  c3361c    jp      j_1c36		; no, jump ahead
...
1bf4  c2361c    jp      nz,j_1c36	; no, jump ahead

j_1bf7:
1bf7  3e01      ld      a,#01		; A := #01
1bf9  cdd01e    call    j_1ed0		; check to see if red ghost is on the edge of the screen (tunnel)
1bfc  381b      jr      c,j_1c19         ; yes, jump ahead

1bfe  3aa74d    ld      a,(#4da7)	; no, load A with red ghost blue flag (0=not blue)
1c01  a7        and     a		; is the red ghost blue (edible) ?
1c02  ca0b1c    jp      z,j_1c0b		; no, skip ahead
1c05  ef        rst     #28		; yes, insert task #0C to control red ghost movement when power pill active
1c06  0c 00
1c08  c3191c    jp      j_1c19		; skip ahead

j_1c0b:
1c0b  2a0a4d    ld      hl,(#4d0a)	; else load HL with red tile position (Y,X)
1c0e  cd5220    call    j_2052		; convert ghost Y,X position in HL to a color screen location
...
1c14  2803      jr      z,j_1c19         ; yes, skip next step

1c16  ef        rst     #28		; no, insert task #08 to control red ghost AI
1c17  08 00

j_1c19:
1c19  cdfe1e    call    j_1efe		; check for and handle red ghost direction reversals
...
1c24  cd0020    call    j_2000		; HL := (IX) + (IY)
...
1c3e  cd0020    call    j_2000		; HL := (IX) + (IY)
1c41  22004d    ld      (#4d00),hl	; store result into red ghost position
1c44  cd1820    call    j_2018		; convert sprite position into a tile position
...
1c5c  cd5a20    call    j_205a		; check to see if pink ghost has entered a tunnel slowdown area
...
1c63  ca7d1c    jp      z,j_1c7d		; no, skip ahead
...
1c98  c3af1c    jp      j_1caf		; skip ahead
...
1cb4  cac41c    jp      z,j_1cc4		; yes, skip ahead
...
1cbe  cace1c    jp      z,j_1cce		; yes, skip ahead

1cc1  c30d1d    jp      j_1d0d		; no, jump ahead
...
1ccb  c20d1d    jp      nz,j_1d0d	; no, skip ahead

j_1cce:
1cce  3e02      ld      a,#02		; yes, A := #02
1cd0  cdd01e    call    j_1ed0		; check to see if pink ghost is on the edge of the screen (tunnel)
1cd3  381b      jr      c,j_1cf0         ; yes, jump ahead

1cd5  3aa84d    ld      a,(#4da8)	; no, load A with pink ghost blue flag (0=not blue)
1cd8  a7        and     a		; is the pink ghost blue ?
1cd9  cae21c    jp      z,j_1ce2		; no, skip ahead

1cdc  ef        rst     #28		; yes, insert task to handle pink ghost movement when power pill active
1cdd  0d 00				; task data
1cdf  c3f01c    jp      j_1cf0		; skip ahead

j_1ce2:
1ce2  2a0c4d    ld      hl,(#4d0c)	; load HL with pink ghost Y,X tile pos
1ce5  cd5220    call    j_2052		; convert ghost Y,X position in HL to a color screen location
...
1ceb  2803      jr      z,j_1cf0         ; yes, skip next step

1ced  ef        rst     #28		; insert task to handle pink ghost AI
1cee  09 00				; task data

j_1cf0:
1cf0  cd251f    call    j_1f25		; check for and handle when pink ghost reverses directions
...
1cfb  cd0020    call    j_2000		; HL := (IX) + (IY)
...
1d15  cd0020    call    j_2000		; HL := (IX) + (IY)
1d18  22024d    ld      (#4d02),hl	; store result into pink ghost postion
1d1b  cd1820    call    j_2018		; convert sprite position into a tile position
...
1d6f  c3861d    jp      j_1d86		; jump ahead
...
1d8b  ca9b1d    jp      z,j_1d9b		; yes, skip ahead
...
1d95  caa51d    jp      z,j_1da5		; yes, skip ahead
1d98  c3e41d    jp      j_1de4		; no, jump ahead
...
1da2  c2e41d    jp      nz,j_1de4	; no, skip ahead

j_1da5:
1da5  3e03      ld      a,#03		; yes, A := #03
1da7  cdd01e    call    j_1ed0		; check to see if inky is on the edge of the screen (tunnel)
1daa  381b      jr      c,j_1dc7         ; yes, jump ahead

1dac  3aa94d    ld      a,(#4da9)	; no, load A with inky blue flag (0 = not blue)
1daf  a7        and     a		; is inky edible ?
1db0  cab91d    jp      z,j_1db9		; no, skip ahead

1db3  ef        rst     #28		; yes, insert task to handle blue ghost (inky) movement when power pill active
1db4  0e 00
1db6  c3c71d    jp      j_1dc7		; skip ahead

j_1db9:
1db9  2a0e4d    ld      hl,(#4d0e)	; load HL with inky tile position
1dbc  cd5220    call    j_2052		; covert to color screen location
...
1dec  cd0020    call    j_2000		; HL := (IX) + (IY)
1def  22044d    ld      (#4d04),hl	; store result into inky position
1df2  cd1820    call    j_2018		; convert sprite position into a tile position
...
1e0a  cd5a20    call    j_205a		; check to see if orange ghost has entered a tunnel slowdown area
...
1e11  ca2b1e    jp      z,j_1e2b		; no, skip ahead
...
1e46  c35d1e    jp      j_1e5d		; skip ahead
...
1e62  ca721e    jp      z,j_1e72		; yes, skip ahead
...
1e6c  ca7c1e    jp      z,j_1e7c		; yes, skip ahead

1e6f  c3bb1e    jp      j_1ebb		; no, jump ahead
...
1e99  2803      jr      z,j_1e9e         ; yes, skip next step

1e9b  ef        rst     #28		; insert task to control orange ghost AI
1e9c  0b 00				; task data

j_1e9e:
1e9e  cd731f    call    j_1f73		; check for and handle when orange ghost reverses directions
...
1ea9  cd0020    call    j_2000		; HL := (IX) + (IY)
...
1ec3  cd0020    call    j_2000		; HL := (IX) + (IY)
1ec6  22064d    ld      (#4d06),hl	; store result into orange ghost position
1ec9  cd1820    call    j_2018		; convert sprite position into a tile position
...
1ef7  d2fc1e    jp      nc,j_1efc	; yes, set carry flag and return
...
1f9f  2804      jr      z,j_1fa5         ; jp, pop, to 3000 if yes

	; not in test mode

1fa1  f1        pop     af		; restore AF
1fa2  c38d00    jp      j_008d		; continue the original handler

	; in test mode

j_1fa5:
1fa5  f1        pop     af		; restore AF
1fa6  c30030    jp      j_3000		; we're in init, continue testing
...
j_200f:
200f  cd0020    call    j_2000		; HL := (IX) + (IY)
2012  cd6500    call    j_0065		; convert to screen position
...
j_2052:
2052  cd6500    call    j_0065		; convert Y,X position to screen position
...
j_205a:
205a  cd5220    call    j_2052		; convert ghost Y,X position in HL to a color screen location
...
JP SLOWMAP
NOP
2060  c36f36    jp      j_366f		; jump to new patch for ms. pac man.  if no tunnel match, returns to #2066
...
20b8  cac920    jp      z,j_20c9		; no, skip ahead
...
20e3  c2f420    jp      nz,j_20f4	; yes, skip ahead
...
2108  c33534    jp      j_3435		; jump to new ms. pac man routine
...
211D: D6 21	sub	#21
211f  200f      jr      nz,j_2130

2121  3c        inc     a
2122  32a04d    ld      (#4da0),a
2125  32b74d    ld      (#4db7),a
2128  cd0605    call    j_0506
...
j_2130:
2130  cd0618    call    j_1806
2133  cd0618    call    j_1806
j_2136:
2136  cd361b    call    j_1b36
2139  cd361b    call    j_1b36
213c  cd230e    call    j_0e23		; change animation of ghosts every 8th frame
213f  c9        ret     

2140  3a3a4d    ld      a,(#4d3a)
2143  d61e      sub     #1e
2145  c23021    jp      nz,j_2130
2148  c32b21    jp      j_212b

214b  3a324d    ld      a,(#4d32)
214e  d61e      sub     #1e
2150  c23621    jp      nz,j_2136

2153  cd701a    call    j_1a70
2156: AF	xor	a		; A: = #00
2157: 32 AC 4E	ld	(#4EAC),a	; clear sound channel 2
215A: 32 BC 4E	ld	(#4EBC),a	; clear sound channel 3
215d  cda505    call    j_05a5
...
216d  c32b21    jp      j_212b

2170  3a324d    ld      a,(#4d32)
2173  d62f      sub     #2f
2175  c23621    jp      nz,j_2136

2178  c32b21    jp      j_212b

217b  3a324d    ld      a,(#4d32)
217e  d63d      sub     #3d
2180  c23021    jp      nz,j_2130

2183  c32b21    jp      j_212b

2186  cd0618    call    j_1806
2189  cd0618    call    j_1806
...
21a1  c34f34    jp      j_344f		; jump to new code for ms. pac-man
...
21df  180f      jr      j_21f0           ; skip ahead

21e1  3a3a4d    ld      a,(#4d3a)
21e4  d62c      sub     #2c
21e6  c23021    jp      nz,j_2130
21e9  3c        inc     a
...
229a  c36934    jp      j_3469		; jump to new code for ms. pac-man
...
22aa  d625	sub 	#25
22ac  c23021    jp      nz,j_2130
22af  3c        inc     a
22b0  32a04d    ld      (#4da0),a
22b3  32b74d    ld      (#4db7),a
22b6  cd0605    call    j_0506
...
22c1  feff      cp      #ff
22c3  2805      jr      z,j_22ca         ; (5)
22c5  fefe      cp      #fe
22c7  c23021    jp      nz,j_2130
j_22ca:
22ca  3c        inc     a
22cb  3c        inc     a
...
22f2  c33021    jp      j_2130

22f5  3a324d    ld      a,(#4d32)
22f8  d61e      sub     #1e
22fa  28bd      jr      z,j_22b9         ; (-67)

22fc  18e6      jr      j_22e4           ; (-26)
...
2324  20fc      jr      nz,j_2322        ; loop #FF times
2326  24        inc     h		; next address (high byte)
2327  10f1      djnz    j_231a           ; Next B
...
2336  20fc      jr      nz,j_2334        ; loop #FF times
2338  24        inc     h		; next high address
2339  10f0      djnz    j_232b           ; Next B
...
2392  fa8d23    jp      m,j_238d		; if sign negative (EG #FF), loop again; nothing to do
...
239c  2002      jr      nz,j_23a0        ; If HL has not reached #4C00 then skip next step
...
23fd  20fc      jr      nz,j_23fb        ; no, loop again
23ff  c9        ret     		; return
...
240a  20fc      jr      nz,j_2408        ; no, loop again
...
2416  20fc      jr      nz,j_2414        ; no, loop again
2418  c9        ret    			; return
...
ORG 241CH
CALL WALLADR
241c  cd6a94    call    j_946a		; ms. pac patch to retreive map info.  loads BC based on the map

j_241f:
241f  0a        ld      a,(bc)		; get maze data
2420  a7        and     a		; == #00 ?
2421  c8        ret     z		; yes, the end of the level data, return
2422  fa2c24    jp      m,j_242c		; if it's < #80, data is an offset.  if not, jump ahead
...
2445  c31f24    jp      j_241f		; loop again
...
244b  c37c94    jp      j_947c		; jump to ms pac patch.  it returns to #2453
...
ORG 2472H
JP DRAWEN
2472  c3ec94    jp      j_94ec		; jump to ms pac patch for power pellet drawing
...
ORG 248AH
JP DOTSA2
248a  c38194    jp      j_9481		; jump to Ms Pac Man patch.  returns to #2492.  Loads IY with start of pellet table based on level
...
24a0  2801      jr      z,j_24a3         ; if equal then skip next step
...
ORG 24B4H
JP READEN
24b4  c30495    jp      j_9504		; jump to ms. pac patch for pellet check routine and return
...
ORG 24DDH
24dd  c38095    jp      j_9580		; jump to new sub to select screen color (ms pac patch.  returns to #24E1)
...
24e9  20fc      jr      nz,j_24e7        ; no, loop again
...
ORG 24F9H
JP SCOLOR
24f9  c3c395    jp      j_95c3		; jump to new ms. pac man patch to color the tunnels with invisible slowdown "paint".  returns to #2534
...
250a  10f6      djnz    j_2502           ; loop until done
...
251f  10f3      djnz    j_2514           ; loop until done
...
2532  10f3      djnz    j_2527           ; loop until done
...
2572  a7        and     a		; == #00 ?
2573  c20f26    jp      nz,j_260f	; no, skip ahead
...
26ae  c2a626    jp      nz,j_26a6	; no, loop again
...
26d6  c2de26    jp      nz,j_26de	; no, skip ahead
...
26f3  2001      jr      nz,j_26f6        ; no, skip next step
...
2743  2013      jr      nz,j_2758        ; no, get normal red direction
...
ORG 274BH
CALL RCORNER
274b  cd6195    call    j_9561		; load DE with a (random ?) quadrant for the destination
274e  cd6629    call    j_2966		; get dir. by finding shortest distance
...
2762  cd6629    call    j_2966		; get best new dir. by finding shortest distance
...
2771  c28e27    jp      nz,j_278e	; no, skip ahead and do pink ghost AI
2774  3a044e    ld      a,(#4e04)	; yes, load A with level cleared register
2777  fe03      cp      #03		; == # 03 ? (why?  when game is played, this is always 3 ???)
2779  2013      jr      nz,j_278e        ; no, skip ahead and do pink ghost AI (never will take this route??)
...
ORG 2781H
CALL RCORNER
2781  cd6195    call    j_9561		; call new code to pick a location to move toward ?
2784  cd6629    call    j_2966		; get new direction by finding shortest distance
...
279f  cd6629    call    j_2966		; compute best new directions
...
27ae  c2cb27    jp      nz,j_27cb	; no ,skip ahead and do normal inky ghost AI
27b1  3a044e    ld      a,(#4e04)	; yes, load A with level cleared register
27b4  fe03      cp      #03		; == # 03 ?  (this always 3 during a game ... ?)
27b6  2013      jr      nz,j_27cb        ; jump if not 3 ahead to do normal AI
...
ORG 2781H
CALL RCORNER
27bb  cd5995    call    j_9559		; pick a random quadrant (why ??? DE is loaded new in next step)
27be  114020    ld      de,#2040	; load DE with lower right corner destination
27c1  cd6629    call    j_2966		; get best new direction
...
27e7  cd6629    call    j_2966		; get best new direction
...
27f6  c21328    jp      nz,j_2813	; no, skip ahead and normal orange ghost AI
27f9  3a044e    ld      a,(#4e04)	; load A with level cleared register
27fc  fe03      cp      #03		; == #03 ?  ( this is always 3 during game)
27fe  2013      jr      nz,j_2813        ; jump if not 3 to normal orange ghost AI
...
ORG 2803H
CALL R2CORNER
2803  cd5e95    call    j_955e		; pick a random quadrant (why?  DE is loaded new in next step)
2806  11403b    ld      de,#3b40	; load DE with lower left corner destination
2809  cd6629    call    j_2966		; get best new direction
...
281b  cdea29    call    j_29ea		; load HL with sum of square of X and Y distances
...
2824  da0028    jp      c,j_2800		; yes, jump back and have ghost move toward lower left corner
...
2831  cd6629    call    j_2966		; get best new direction
...
283f  ca5528    jp      z,j_2855		; yes, skip ahead and give random direction
...
284b  cd6629    call    j_2966		; get best new direction
...
285b  cd1e29    call    j_291e		; load A and HL with random direction and tile direction
...
2869  ca7f28    jp      z,j_287f		; yes, skip ahead and give random direction
...
2875  cd6629    call    j_2966		; get best new direction
...
2885  cd1e29    call    j_291e		; load A and HL with random direction and tile direction
...
2893  caa928    jp      z,j_28a9		; yes, skip ahead and give random direction
...
289f  cd6629    call    j_2966		; get best new direction
...
28af  cd1e29    call    j_291e		; load A and HL with random direction and tile direction
...
28bd  cad328    jp      z,j_28d3		; yes, skip ahead and assign random direction
...
28c9  cd6629    call    j_2966		; get best new directions
...
28d9  cd1e29    call    j_291e		; load A and HL with random direction and tile direction
...
28e7  cafe28    jp      z,j_28fe		; no, skip ahead
...
28f4  cd6629    call    j_2966		; get best new direction
...
2914  cd6629    call    j_2966		; get best new direction
...
2926  cd232a    call    j_2a23		; load A with a pseudo random number
...
2941  ca5729    jp      z,j_2957		; yes, skip ahead to choose a new direction

2944  cd0f20    call    j_200f		; no, load A with the character in the destination screen position
2947  e6c0      and     #c0		; mask bits
2949  d6c0      sub     #c0		; subtract. is there a wall in the way of this direction ?
294b  280a      jr      z,j_2957		; yes, choose a new direction and try again
...
2963  c33d29    jp      j_293d		; jump back
...
298c  cac629    jp      z,j_29c6		; yes, skip ahead and try next position

298f  cd0020    call    j_2000		; no, HL := (IX) + (IY)
2992  22424d    ld      (#4d42),hl	; store into temp position
2995  cd6500    call    j_0065		; convert to screen position
...
299d  2827      jr      z,j_29c6         ; yes, skip ahead and try next position
...
29ab  cdea29    call    j_29ea		; load HL with sum of the square of the X and Y distances between the 2 positions
...
29b9  dac629    jp      c,j_29c6		; no, skip ahead and try next position
...
29d1  c28829    jp      nz,j_2988	; no, loop again and try more
...
29f1  d2f929    jp      nc,j_29f9	; yes, skip ahead
...
j_29f9:
29f9  cd122a    call    j_2a12		; result should always be a small number.  HL := A * A
...
2a04  d20c2a    jp      nc,j_2a0c	; yes, skip ahead
...
j_2a0c:
2a0c  cd122a    call    j_2a12		; HL = A * A
...
2a1a  d21e2a    jp      nc,j_2a1e	; no carry, skip next step

2a1d  19        add     hl,de		; add result to DE

j_2a1e:
2a1e  0d        dec     c		; decrease counter
2a1f  c2192a    jp      nz,j_2a19	; loop if not done, 8 times
2a22  c9        ret     		; return
...
2a3f  1a        ld      a,(de)
2a40  fe10      cp      #10
2a42  ca532a    jp      z,j_2a53

2a45  fe12      cp      #12
2a47  ca532a    jp      z,j_2a53

2a4a  fe14      cp      #14
2a4c  ca532a    jp      z,j_2a53

2a4f  13        inc     de
2a50  c3382a    jp      j_2a38

j_2a53:
2a53  3e40      ld      a,#40
2a55  12        ld      (de),a
2a56  13        inc     de
2a57  c3382a    jp      j_2a38
...
2a65  cd0b2b    call    j_2b0b		; load HL with score address for current player
...
2a83  dc332b    call    c,j_2b33		; if yes, call sub to continue check for extra life
2a86  cdaf2a    call    j_2aaf		; draw player score onscreen
2a89  13        inc     de		; 
2a8a  13        inc     de
...
2a94  2005      jr      nz,j_2a9b        ; if they are equal, continue, else update the high score
2a96  1b        dec     de		; next digit
2a97  2b        dec     hl		; next digit
2a98  10f7      djnz    j_2a91           ; next B
2a9a  c9        ret     		; return

	; arrive when player score beats the current high score

j_2a9b:
2a9b  cd0b2b    call    j_2b0b		; load HL with score address for current player
...
2aad  180f      jr      j_2abe           ; draw high score to screen and return
...
2ab9  2803      jr      z,j_2abe         ; yes, skip ahead
...
2ac3  cdce2a    call    j_2ace		; drawtens digit to screen
2ac6  1a        ld      a,(de)		; load A with byte of score
2ac7  cdce2a    call    j_2ace		; draw ones digit to screen
2aca  1b        dec     de		; next score digit
2acb  10f1      djnz    j_2abe           ; loop 3 times
2acd  c9        ret     		; return

j_2ace:
2ace  e60f      and     #0f		; mask out left 4 bits to zero
2ad0  2804      jr      z,j_2ad6         ; result zero?  yes, skip next 2 steps

2ad2  0e00      ld      c,#00		; C := #00
2ad4  1807      jr      j_2add           ; skip ahead

j_2ad6:
2ad6  79        ld      a,c		; load A with C
2ad7  a7        and     a		; == #00 ?
2ad8  2803      jr      z,j_2add         ; yes, skip ahead
...
2ae2  cd5e2c    call    j_2c5e		; print HIGH SCORE
2ae5  af        xor     a		; A := #00
...
2af5  cdbe2a    call    j_2abe		; draw score to screen
...
2b05  20b7      jr      nz,j_2abe        ; no, draw player 2 score and return
2b07  0e06      ld      c,#06		; else C := #06
2b09  18b3      jr      j_2abe           ; draw player 2 score and return
...
2b50  a7        and     a		; == #00 ?
2b51  280e      jr      z,j_2b61         ; yes, skip ahead, nothing to draw

2b53  fe06      cp      #06		; >= #06 ?
2b55  300a      jr      nc,j_2b61        ; yes, skip ahead, we can't draw more than 5 extra lives

j_2b57:
2b57  3e20      ld      a,#20		; A := #20
2b59  cd8f2b    call    j_2b8f		; draw extra life
2b5c  2b        dec     hl		; 
...
2b70  cdcd2b    call    j_2bcd		; colors the bottom two rows of 10 the color 9 (yellow)
...
2b7c  18cc      jr      j_2b4a           ; draw extra lives on screen and return
...
2ba6  2005      jr      nz,j_2bad        ; no? then skip ahead
2ba8  0602      ld      b,#02		; load code for "FREE PLAY"
2baa  c35e2c    jp      j_2c5e		; print FREE PLAY and return from sub

j_2bad:
2bad  0601      ld      b,#01		; else load code for "CREDIT"
2baf  cd5e2c    call    j_2c5e		; print "CREDIT" on screen
2bb2  3a6e4e    ld      a,(#4e6e)	; load A with number of credits in ram
2bb5  e6f0      and     #f0		; mask bits.  is it bigger than 9?
2bb7  2809      jr      z,j_2bc2         ; yes, only draw 1 position
2bb9  0f        rrca  			; else ...  
2bba  0f        rrca    		;
2bbb  0f        rrca    		; 
...
2be7  20f4      jr      nz,j_2bdd        ; No, loop again
2be9  c9        ret     		; return
...
!    ORG 2BF4H
!    JP MAXFRUIT
2bf4  c39387    jp      j_8793		; jump to new ms. pac man sub, returns to #2BF9
...
2C03 CD8F2B 	CALL	j_2b8f 		; Draw fruit subroutine
2C06 3E04 	LD	A,#04 		; 
...
2C0C CD802B 	CALL	j_2b80 		; Draw color subroutine
2C0F 3EFC 	LD	A,#FC 		; 
...
2C2C 18EB 	JR	j_2c19 		; Jump back and draw next clear
...
2C30 3802 	JR	C,j_2c34 	; If not, skip next step
...
2C41 C3FD2B 	JP	j_2bfd 		; Jump back up to fruit drawing section
...
2c77  2003      jr      nz,j_2c7c        ; yes, skip next step
...
2c82  3828      jr      c,j_2cac         ; special draw for entries 80+
...
2c87  2809      jr      z,j_2c92         ; yes, done with VRAM, skip ahead to color
...
2c96  a7        and     a		; > #80 ?
2c97  faa42c    jp      m,j_2ca4		; yes, skip ahead
...
2ca1  10f7      djnz    j_2c9a           ; loop until b==0
2ca3  c9        ret     		; return
...
2ca9  10f9      djnz    j_2ca4           ; loop until b==0
2cab  c9        ret     
...
2caf  280a      jr      z,j_2cbb         ; yes, done with vram
...
#if MSPACMAN
j_2cc1:
2cc1  jp      j_9797       		; sprite/cocktail stuff. we don't care for sound.
...
2ccc  call    j_2d44                     ; call process_wave
...
2d17  call    j_2dee                     ; call process effect, returns volume in A
...
2d47  and     a
2d48  jp      z,j_2df4           ; then goto init_param
...
2d51  and     c
2d52  jr      nz,j_2d59          ; found one, goto process wave bis
2d54  srl     e
2d56  djnz    j_2d50
2d58  ret                       ; return
...
2d5c  and     e
2d5d  jr      nz,j_2d66          ; if (CUR_BIT & E != 0) then goto #2d66
2d5f  ld      (ix+#02),e        ; else save E in CUR_BIT
2d62  jp      j_364e             ; jump to new ms. pac man routine.  returns to #2D72

2d65  inc     c                 ; junk from pac-man

j_2d66:
2d66  dec     (ix+#0c)          ; decrement W_DURATION
2d69  jp      nz,j_2dd7          ; if W_DURATION == 0
...
2d77  ld      (ix+#07),h
2d7a  cp      #f0               ; if (A < #F0)
2d7c  jr      c,j_2da5           ; then process A  (regular byte)
...
2da6  and     #1f
2da8  jr      z,j_2dad           ; if (A & 0x1f == 0)
2daa  ld      (ix+#0d),b        ; then W_DIR = B
j_2dad:
2dad  ld      c,(ix+#09)        ; C = W_9
2db0  ld      a,(ix+#0b)
2db3  and     #08
2db5  jr      z,j_2db9           ; if (W_8 & 0x8 == 0)
...
2dca  and     #1f
2dcc  jr      z,j_2dd7           ; if (A & 0x1f == 0) then goto compute_wave_freq
...
2ddf  and     #10
2de1  jr      z,j_2de5           ; if (W_DIR & 0x10 != 0) then
2de3  ld      a,#01             ;       A = 1
j_2de5:
2de5  add     a,(ix+#04)        ; A += W_4

2de8  jp      z,j_2ee8           ; compute new frequency  FREQ = BASE_FREQ * (1 << A)
2deb  jp      j_2ee4
...
2df1    and     a               ;
2df2    jr      nz,j_2e1b        ; then goto find effect
...
2e21  and     c
2e22  jr      nz,j_2e29          ; found one, goto proces effect bis
2e24  srl     e
2e26  djnz    j_2e20
2e28  ret
...
2e2c  and     e
2e2d  jr      nz,j_2e6e          ; if (CUR_BIT & E != 0) then goto 2e6e
...
2e63  and     #08
2e65  jr      nz,j_2e6e          ; if (E_TYPE & 0x8 == 0) then
...
2e71  jr      nz,j_2ecd          ; if (E_DURATION == 0) then
2e73  ld      a,(ix+#08)
2e76  and     a
2e77  jr      z,j_2e89           ;       if (E_TABLE5 != 0) then
2e79  dec     (ix+#08)          ;               E_TABLE5--
2e7c  jr      nz,j_2e89          ;               if (E_TABLE5 == 0) then
2e7e  ld      a,e
2e7f  cpl
2e80  and     (ix+#00)
2e83  ld      (ix+#00),a        ;                       E_NUM &= ~E_CUR_BIT
2e86  jp      j_2dee             ;                       goto process effect (one voice)
j_2e89:
2e89  ld      a,(ix+#06)
2e8c  and     #7f
2e8e  ld      (ix+#0c),a        ;       E_DURATION = E_TABLE3 & 0x7F
2e91  bit     7,(ix+#06)
2e95  jr      z,j_2ead           ;       if (E_TABLE3 & 0x80 != 0) then
2e97  ld      a,(ix+#05)
2e9a  neg
2e9c  ld      (ix+#05),a        ;               E_TABLE2 = - E_TABLE2
2e9f  bit     0,(ix+#0d)        ;               if (E_DIR & 0x1 == 0) then
2ea3  set     0,(ix+#0d)        ;                       E_DIR |= 0x1
2ea7  jr      z,j_2ecd           ;                       goto update_freq
...
2ec3  ld      a,(ix+#0b)
2ec6  and     #08
2ec8  jr      nz,j_2ecd          ;       if (E_TYPE & 0x8 == 0) then
...
3010  fe02      cp      #02		; < #02 ?
3012  d20930    jp      nc,j_3009	; no, loop again

3015  24        inc     h		; yes, increase H
3016  10ee      djnz    j_3006           ; next B
...
3020  fe30      cp      #30		; == #30 ?
3022  c20330    jp      nz,j_3003	; no, loop back and continue for other roms
...
302b  da0330    jp      c,j_3003		; no, loop again

302e  c34230    jp      j_3042		; yes, skip ahead for RAM test
...
303f  c3bd30    jp      j_30bd		; skip ahead
...
3057  c24d30    jp      nz,j_304d	; no, loop again
...
3063  c24d30    jp      nz,j_304d	; no, loop again

3066  24        inc     h		; yes, next high byte
3067  15        dec     d		; decrement counter.  are we done ?
3068  c24a30    jp      nz,j_304a	; no, loop again
...
307b  c2b530    jp      nz,j_30b5	; no, RAM test failed, jump ahead for bad RAM
...
3085  c27530    jp      nz,j_3075	; no, loop again
...
3091  c27530    jp      nz,j_3075	; no, loop again

3094  24        inc     h		; yes, next high byte
3095  15        dec     d		; decrement counter.  are we done ?
3096  c27230    jp      nz,j_3072	; no, loop again
...
30a0  47        ld      b,a		; B := A
30a1  10a4      djnz    j_3047           ; loop until done
...
30cc  20fb      jr      nz,j_30c9        ; no, loop again

30ce  24        inc     h		; next high byte
30cf  10f5      djnz    j_30c6           ; next B
...
30dd  20fc      jr      nz,j_30db        ; loop until zero

30df  24        inc     h		; next high byte
30e0  10f4      djnz    j_30d6           ; Next B
...
30eb  20fc      jr      nz,j_30e9        ; loop until zero

30ed  24        inc     h		; next high byte
30ee  10f4      djnz    j_30e4           ; next B
...
30f1  1008      djnz    j_30fb           ; Decrease B.  was there an error?  Yes, jump ahead
...
30f5  cd5e2c    call    j_2c5e		; print to screen
30f8  c37431    jp      j_3174		; jump ahead to test mode
...
3134  2011      jr      nz,j_3147        ; no, skip ahead
...
314f  28f6      jr      z,j_3147         ; no, loop forever

3151  c30b23    jp      j_230b		; yes, jump back to program
...
317b  20fc      jr      nz,j_3179        ; loop until zero
...
319e  2805      jr      z,j_31a5         ; if no credits, skip to next test
...
31ac  2805      jr      z,j_31b3         ; if start buttons not pressed, skip to next test
...
31b7  2805      jr      z,j_31be         ; if not, skip ahead to next test
...
31c2  2805      jr      z,j_31c9         ; if not, skip to next test
...
31cd  2805      jr      z,j_31d4         ; if not, skip to next test
...
31d8  2805      jr      z,j_31df         ; if not, skip to next test
...
31e7  cd5e2c    call    j_2c5e		; print "FREE PLAY" or "1 COIN 1 CREDIT" etc, based on what the DIP settings are
...
31f5  2008      jr      nz,j_31ff        ; no, skip ahead

31f7  062a      ld      b,#2a		; load B with code for "BONUS NONE"
31f9  cd5e2c    call    j_2c5e		; print
31fc  c31c32    jp      j_321c		; skip ahead for next test
...
3204  cd5e2c    call    j_2c5e		; print
3207  062e      ld      b,#2e		; load B with code for "000"
3209  cd5e2c    call    j_2c5e		; print
...
3227  2001      jr      nz,j_322a        ; no, skip next step
...
3251  20fc      jr      nz,j_324f        ; loop until zero
...
3265  10f8      djnz    j_325f           ; loop until done
...
328e  28f6      jr      z,j_3286         ; no, loop until test switch is off
...
3295  c24b23    jp      nz,j_234b	; no, jump to main program start

3298  0608      ld      b,#08		; For B = 1 to 8

j_329a:
329a  cded32    call    j_32ed		; call the delay routine
329d  10fb      djnz    j_329a           ; Next B

329f  3a4050    ld      a,(#5040)	; load A with IN1
32a2  e610      and     #10		; is the service mode switch set ?
32a4  c24b23    jp      nz,j_234b	; no, jump to main program start
...
32ae  cded32    call    j_32ed		; call the delay routine
32b1  3a0050    ld      a,(#5000)	; load A with IN0 (joystick)
32b4  a3        and     e		; pushing on joystick in proper direction?
32b5  20f4      jr      nz,j_32ab        ; no, jump back

j_32b7:
32b7  cded32    call    j_32ed		; yes, call the delay routine
...
32c2  20f3      jr      nz,j_32b7        ; no, loop again
32c4  10e5      djnz    j_32ab           ; Next B
...
32cb  daa932    jp      c,j_32a9		; no, loop and try again
...
32d7  20fc      jr      nz,j_32d5        ; loop until L is zero

32d9  24        inc     h		; next H
32da  10f7      djnz    j_32d3           ; next B
32dc  cdf43a    call    j_3af4		; draw the easter egg to the screen
...
32e7  cadf32    jp      z,j_32df		; yes, loop
32ea  c34b23    jp      j_234b		; no, jump to main program
...
343a  ca9c34    jp      z,j_349c		; yes, skip ahead
...
344c  c39c34    jp      j_349c		; jump ahead
...
3454  ca9c34    jp      z,j_349c		; yes, skip ahead
...
3466  c39c34    jp      j_349c		; jump ahead
...
346e  ca9c34    jp      z,j_349c		; yes, skip ahead
...
34a0  cc1136    call    z,j_3611		; no, call this sub to get it started
...
34b0  fef0      cp      #f0		; == #F0 ?
34b2  cade34    jp      z,j_34de		; handle code #F0 - LOOP
34b5  fef1      cp      #f1
34b7  ca6b35    jp      z,j_356b		; handle code #F1 - SETPOS
34ba  fef2      cp      #f2
34bc  ca9735    jp      z,j_3597		; handle code #F2 - SETN
34bf  fef3      cp      #f3
34c1  ca7735    jp      z,j_3577		; handle code #F3 - SETCHAR
34c4  fef5      cp      #f5
34c6  ca0736    jp      z,j_3607		; handle code #F5 - PLAYSOUND
34c9  fef6      cp      #f6
34cb  caa435    jp      z,j_35a4		; handle code #F6 - PAUSE
34ce  fef7      cp      #f7
34d0  caf335    jp      z,j_35f3		; handle code #F7 - SHOWACT ?
34d3  fef8      cp      #f8
34d5  cafd35    jp      z,j_35fd		; handle code #F8 - CLEARACT ?
34d8  feff      cp      #ff
34da  cacb35    jp      z,j_35cb		; handle code #FF - END
...
34e7  79        ld      a,c
34e8  84        add     a,h
34e9  cd5635    call    j_3556
34ec  12        ld      (de),a
34ed  cd4136    call    j_3641
34f0  df        rst     #18
34f1  7c        ld      a,h
34f2  81        add     a,c
34f3  12        ld      (de),a
...
34fe  79        ld      a,c
34ff  85        add     a,l
3500  cd5635    call    j_3556
3503  1b        dec     de
3504  12        ld      (de),a
3505  cd4136    call    j_3641
3508  df        rst     #18
3509  7d        ld      a,l
350a  81        add     a,c
350b  1b        dec     de
...
351f  c22635    jp      nz,j_3526	; no, skip ahead

3522  0e00      ld      c,#00		; else reset counter
3524  18ef      jr      j_3515           ; loop again
...
3550  2062      jr      nz,j_35b4        ; (98)

3552  1e04      ld      e,#04
3554  185e      jr      j_35b4           ; (94)
...
j_356b:
356b  eb        ex      de,hl
356c  cd4136    call    j_3641		; load HL with either #4CFE or #4Dc6
356f  eb        ex      de,hl
3570  d5        push    de
3571  23        inc     hl
3572  56        ld      d,(hl)
...
3588  1800      jr      j_358a		; does nothing (?) -- jumps to next instruction
...
35c1  dd2b      dec     ix
35c3  dd2b      dec     ix
35c5  1001      djnz    j_35c8           ; (1)
35c7  c9        ret     

j_35c8:
35c8  c3a934    jp      j_34a9
...
3615  2008      jr      nz,j_361f        ; no, skip next 3 steps.  no sounds during attract mode
...
3651: FE 01	cp	#01		; == #01 ?
3653: 28 04	jr	z,j_3659		; yes, skip next 2 steps
3655: 06 00	ld	b,#00		; else B := #00
3657: 18 11	jr	j_366a		; jump ahead
...
366C: C3 72 2D	jp	j_2d72		; jump back to main program to "process byte" routine
...
3671  ca6620    jp      z,j_2066		; no, jump back and set the var to zero
...
368b  83        add     a,e		; add
368c  fe18      cp      #18		; == #18 ?
368e  2002      jr      nz,j_3692        ; no, skip next step
...
3b02  d2fa3a    jp      nc,j_3afa	; no, loop again
3b05  24        inc     h		; yes, increase H
3b06  18f2      jr      j_3afa           ; and loop again
...
3e61  c4d03e    call    nz,j_3ed0	; no, call this sub.  it controls the flashing bulbs around the marquee
...
3ee4  2833      jr      z,j_3f19         ; if zero then jump ahead to do other part of routine
...
3f1c  fa213f    jp      m,j_3f21		; if negative, skip next step
...
8012  c9        ret
8013  c37836    jp      j_3678
8016  3a00

; - OVERLAY - 0x2108

8018  c33534    jp      j_3435
801b  e7        rst     #20
801c  1a        ld      a,(de)
...
8053  cd6195    call    j_9561
8056  cd66--    call    #--66

; - OVERLAY - 0x2448

8058  210040    ld      hl,#4000
805b  c37c94    jp      j_947c
805e  4e        ld      c,(hl)
805f  fd
...
808a  c9        ret     
808c  c39c86    jp      j_869c
808f  c9        ret     

; - OVERLAY - 0x274a

2748  ----4d    ld      a,(#4d2c)
8091  cd6195    call    j_9561
8094  cd6629    call    j_2966
8097  22---	ld      ...
...
80c1  fe02      cp      #02
80c3  3e1f      ld      a,#1f
80c5  c38095    jp      j_9580

; - OVERLAY - 0x16d8

80c8  ----4d    ld      a,(#4d09)
80c9  c3c586    jp      j_86c5
80cc  c9        ret     
80cd  3808      jr      c, +#08
80cf  1e--      ld      e,#--
...
8142  c3ec94    jp      j_94ec
8145  eda0      ldi     
8147  11----    ld      de,#6fc3

; - OVERLAY - 0x2060

8148  c36f36    jp      j_366f   
2063  00        nop
814c  02        ld      (bc),a
814d  c9        ret     
814e  af        xor     a
...
8184  c30495    jp      j_9504
8187  ed--      ldi

; - OVERLAY - 0x16b0

8187  c9        ret
8189  c3b186    jp      j_86b1
818c  c9        ret     
818d  07        rlca    
818e  fe06      cp      #06
...
819b  77        ld      (hl),a
819c  c9        ret     
819d  c3ee86    jp      j_86ee

; - OVERLAY - 0x21a0

81a0  ----4e    ld      a,(#4e07)
81a1  c34f34    jp      j_344f
81a4  41        ld      b,c
81a5  e7        rst     #20
81a6  c221--    jp      nz,#--21
...
81d3  3c        inc     a
81d4  c39387    jp      j_8793
81d7  2e--      ld      l,#4e		; junk

; - OVERLAY - 0x08e0

81d8  ----4e    ld      a,(#4e0e)
81d9  c3a194    jp      j_94a1
81dc  00        nop     
...
86a9  2002      jr      nz,j_86ad        ; yes, skip next step
...
86bd  2002      jr      nz,j_86c1        ; yes, skip next step
...
86d1  2002      jr      nz,j_86d5        ; yes, skip next step
...
86e6  2002      jr      nz,j_86ea        ; yes, skip next step
...
86F6: A7	and	a		; == #00 ?
86F7: CA 47 87	jp	z,j_8747		; yes, check for fruit release

86FA: 3A D2 4D	ld	a,(#4DD2)	; load A with fruit X position
86FD: A7	and	a		; is a fruit already onscreen?
86FE: CA 47 87	jp	z,j_8747		; no, then jump to check for 
...
871d  fab587    jp      m,j_87b5		; if negative, jump out to this subroutine
...
8733  2807      jr      z,j_873c         ; if zero, skip next 4 steps

j_8735:
8735  cb39      srl     c
8737  cb39      srl     c		; shift C right twice
8739  3d        dec     a		; A := A - 1.  is A == #00 ?
873a  20f9      jr      nz,j_8735        ; no, loop again
...
8764  380a      jr      c,j_8770         ; If less than 7, use board # as fruit
...
876d  30fd      jr      nc,j_876c        ; If >=0 loop
...
8787  cdcd87    call    j_87cd		; set up fruit path
...
8795 DA F9 2B 	JP 	C,j_2bf9 	; No, return
8798 3E 07 	LD 	A,#07 		; Yes, set A := #07
879A C3 F9 2B 	JP 	j_2bf9 		; Return
...
87ba  fe40      cp      #40		; > 40 ?
87bc  3852      jr      c,j_8810         ; yes, jump ahead and return
...
87c8  2023      jr      nz,j_87ed        ; If not zero then jump ahead

87ca  210088    ld      hl,#8800	; else if zero then load HL with start of data table for fruit exit

j_87cd:
87cd  cdbd94    call    j_94bd		; load BC with valued from table based on level
87d0  69        ld      l,c		; 
...
87f5  c3e487    jp      j_87e4		; jump back
...
8815  c30010    jp      j_1000		; jump back to program (clears #4DD4 and returns from sub)
...
8823: 30 18	jr	nc,j_883d	; no , jump back to program
...
882B: 30 10	jr	nc,j_883d	; no, jump back to program
...
883A: C3 B2 19	jp	j_19b2		; jump back to program for fruit being eaten

j_883d:
883D: F1	pop	af		; Restore AF
883E: C3 CD 19	jp	j_19cd		; jump back to program with no fruit eaten
...
946d  cdbd94    call    j_94bd		; load BC based on the maze
...
947f  1803      jr      j_9484           ; skip next step
...
9488  cdbd94    call    j_94bd		; load BC with value based on the level
...
94a5  cdbd94    call    j_94bd		; load BC based on board #
...
94af  c2eb08    jp      nz,j_08eb	; if not done, return to the game loop
94b2  c3e508    jp      j_08e5		; else return to the program and signal end of level
...
94C3 F2 D4 94 	JP	P,j_94d4 	; Yes, jump to subroutine to makes the result in A become between 0 and #0D [Bug, should be JP NC, not JP P]
j_94c6:
...
94D8 F2 D6 94 	JP 	P,j_94d6 	; Yes, then repeat previous subtraction [Bug, should be JP NC, not JP P]
94DB C6 0D 	ADD 	A,#0D 		; No, add #0D (13 decimal) back into A
94DD 18 E7 	JR 	j_94c6 		; Return to program
...
94ef  cdbd94    call    j_94bd		; load BC with value based on level from the table
...
9501  20f4      jr      nz,j_94f7        ; if not zero, loop again
9503  c9        ret     		; return
...
9507  cdbd94    call    j_94bd		; load BC with value based on level from the table
...
9519  20f4      jr      nz,j_950f        ; if not zero, loop again
...
9529  cdbd94    call    j_94bd		; load BC with address of power pellet table based on map played
952c  60        ld      h,b
...
9538  2002      jr      nz,j_953c        ; if not zero then skip next step
...
955c  1803      jr      j_9561           ; skip ahead to pick a destination
...
9567  cdbd94    call    j_94bd		; load BC based on level and HL
956a  69        ld      l,c		; 
...
j_9580:
9580  cae124    jp      z,j_24e1		; if zero then return immediately, used for color white flashing at end of level
9583  3a024e    ld      a,(#4e02)	; load A with main routine 1, subroutine #
9586  a7        and     a		; == #00 ?  
9587  2807      jr      z,j_9590         ; yes, skip ahead to select the color to use based on the board number
9589  fe10      cp      #10		; == #10 ?  Is the game in the demo maze ?
958b  3e01      ld      a,#01		; load A with 1.  used to properly color the midway logo
958d  c2e124    jp      nz,j_24e1	; no, return to program
...
9595 F2 A3 95 	JP 	P,j_95a3 	; Yes, go and bring it back down to a number between #5 and #15 [Bug.  should JP NC, not JP P]
j_9598:
...
95A0 C3 E1 24 	JP 	j_24e1		; Jump back to program

j_95a3:
95A3 D6 15 	SUB 	#15 		; Subtract #15 from A
j_95a5:
95A5 D6 10 	SUB 	#10 		; Subtract #10 from A
95A7 F2 A5 95 	JP 	P,j_95a5 	; Did we just go negative? No, go back and subtract another 10.  [Bug.  should be JP NC, not JP P]
95AA C6 15 	ADD 	A,#15 		; Yes, Add #15 back into A
95AC 18 EA 	JR 	j_9598 		; Return
...
95C8 F2 34 25 	JP 	P,j_2534 	; No, jump back to program [bug.  should be JP NC, not JP P.]
95CB 21 DF 95 	LD 	HL,#95DF 	; Yes, load HL with start of table data address
95CE CD BD 94 	CALL 	j_94bd 		; Load BC with either #95DF or #95E1 depending on the level
...
95D7 CA 34 25 	JP 	Z,j_2534 	; Yes, jump back to program
...
95DD 18 F5 	JR 	j_95d4 		; Loop back and do again
...
95E4: FE 0A	cp	#0A		; == #0A ?
95e6  cc0b96    call    z,j_960b		; Yes, draw the MS PAC MAN graphic which appears between "ADDITIONAL" and "AT 10,000 pts"
95e9  fe0b      cp      #0b		; == #0B ?
95eb  ccf695    call    z,j_95f6		; yes, draw midway logo and copyright text
95ee  fe06      cp      #06		; == #06 ?   ( code for "READY!" )
95f0  cc3c96    call    z,j_963c		; yes, clear the intermission indicator
95f3  c35e2c    jp      j_2c5e		; jump to print routine

	
j_95f6:
95f6  c5        push    bc		; save BC
95f7  e5        push    hl		; save HL
95f8  cd4296    call    j_9642		; draw the midway logo and copyright text for the 'press start' screen
...
9610  cd2796    call    j_9627		; draws the MS PAC MAN graphic which appears between "ADDITIONAL" and "AT 10,000 pts" 
...
962a  280f      jr      z,j_963b         ; yes, return
...
9639  18ec      jr      j_9627           ; loop again
j_963b:
963b  c9        ret     		; return
...
967a  20d8      jr      nz,j_9654        ; No, loop again
967c  c9        ret     		; return
...
979c  280b      jr      z,j_97a9         ; no, skip next 4 steps
...
97c1  c3c42c    jp      j_2cc4		; jump back to program
...
981F: C6 03         add  a,#03
9821: FE 06         cp   #06
9823: 30 18         jr   nc,j_983d

9825: 7D            ld   a,l
9826: 93            sub  e
9827: C6 03         add  a,#03
9829: FE 06         cp   #06
982B: 30 10         jr   nc,j_983d
...
9838: D6 02         sub  #02
983A: C3 B2 19      jp   j_19b2

j_983d:
983D: F1            pop  af
983E: C3 CD 19      jp   j_19cd
